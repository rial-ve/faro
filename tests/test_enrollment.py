...
async def test_get_enroll_with_valid_token_returns_form():
    async with _client() as ac:
        token = (await ac.post("/v1/enrollment-tokens", json={"label": ""})).json()["token"]
        r = await ac.get(f"/enroll/{token}")
    assert r.status_code == 200
...
async def test_get_enroll_with_invalid_token_returns_invalid_page():
    async with _client() as ac:
        r = await ac.get("/enroll/not-a-real-token")
    assert r.status_code == 200
...
async def test_post_enroll_with_valid_token_creates_pending_person():
    async with _client() as ac:
        token = (await ac.post("/v1/enrollment-tokens", json={}).json())["token"] if False else (
            await ac.post("/v1/enrollment-tokens", json={})
        ).json()["token"]
        ...
        pending = (await ac.get("/v1/persons", params={"status": "pending"})).json()
        active = (await ac.get("/v1/persons", params={"status": "active"})).json()
...
async def test_post_enroll_with_invalid_token_does_not_create_person():
    async with _client() as ac:
        ...
        assert r.status_code == 200
        assert "inválido" in r.text.lower() or "expirado" in r.text.lower()
        persons = (await ac.get("/v1/persons")).json()
    assert persons == []


async def test_post_enroll_with_no_face_re_renders_form_with_error():
    async with _client() as ac:
        token = (await ac.post("/v1/enrollment-tokens", json={})).json()["token"]
        r = await ac.post(
            f"/enroll/{token}",
            data={"name": "X", "description": "y"},
...
async def test_approve_changes_status_from_pending_to_active():
    async with _client() as ac:
        ...
        r = await ac.post(f"/v1/persons/{person_id}/approve")
        assert r.status_code == 200
...
async def test_recognize_matches_after_approval():
    async with _client() as ac:
        ...
        pending = (await ac.get("/v1/persons", params={"status": "pending"})).json()
        await ac.post(f"/v1/persons/{pending[0]['id']}/approve")
